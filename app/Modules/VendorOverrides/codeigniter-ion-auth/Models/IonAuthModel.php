...
class IonAuthModel
{
	...
	/**
	 * Clear the signup password code for a user
	 *
	 * @param string $identity Identity
	 *
	 * @return boolean Success
	 */
	public function clearSignupPasswordCode(string $identity): bool
	{
		if (empty($identity))
		{
			return false;
		...
		$data = [
			'signup_password_selector' => null,
			'signup_password_code'     => null,
			'signup_password_time'     => null,
		];
	 ...
	 * @author Ryan
	 */
	public function setPasswordByUser(string $identity)
	{
		if (empty($identity))
		{
			/*$this->triggerEvents(['post_forgotten_password', 'post_forgotten_password_unsuccessful']);*/
		...
		$update = [
			'signup_password_selector' => $token->selector,
			'signup_password_code'     => $token->validatorHashed,
			'signup_password_time'     => time(),
		];
	...
	/**
	 * Get a user from a signup password key.
	 *
	 * @param string $userCode Forgotten password key
	 *
	 * @return  boolean|object
	 ...
	 * @updated Ryan
	 */
	public function getUserBySignupPasswordCode(string $userCode)
	{
		// Retrieve the token object from the code
		$token = $this->retrieveSelectorValidatorCouple($userCode);
		// Retrieve the user according to this selector
		$user = $this->where('signup_password_selector', $token->selector)->users()->row();

		if ($user)
		{
			// Check the hash against the validator
			if (isset($user->signup_password_code) && $this->verifyPassword($token->validator, $user->signup_password_code))
			{
				return $user;
			}
		}
	...
	public function login(string $identity, string $password, bool $remember=false): bool
	{
		...
		$query = $this->db->table($this->tables['users'])
						  ->select($this->identityColumn . ', email, id, password, active, last_login,group_id')
						  ->where($this->identityColumn, $identity)
						  ->limit(1)
						  ->orderBy('id', 'desc')
						  ->get();
		...
		if (isset($user))
		{
			if ($this->verifyPassword($password, $user->password, $identity))
			{
				...
				$this->clearSignupPasswordCode($identity);
	...
	public function getUsersGroups(int $id=0)
	{
		...
		return $builder->select($this->tables['users_groups'] . '.' . $this->join['groups'] . ' as id, ' . $this->tables['groups'] . '.name, ' . $this->tables['groups'] . '.code, ' . $this->tables['groups'] . '.description')
	...
	public function setSession(\stdClass $user): bool
	{
		...
		$sessionData = [
			...
			'last_check'          => time(),
			'user_type'			=> $this->group($user->group_id)->row()->code,
		];
