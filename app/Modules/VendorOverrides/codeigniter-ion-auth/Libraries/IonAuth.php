...
class IonAuth
{
		...
		if ($user)
		{
			...
			if ($code)
			{
				...
				else
				{
					...
					//$this->email->setTo('karthik.hbk24@gmail.com');
	...
	public function setPasswordByUser(string $identity)
	{
		...
		{
			// Generate code
			$code = $this->ionAuthModel->setPasswordByUser($identity);
			if ($code)
			{
				$data = [
					'identity'              => $identity,
					'signup_password_code' => $code,
				];
				...
				{
					$message = view($this->config->emailTemplates . 'signup_password.php', $data);
					$this->email->clear();
					$this->email->setFrom($this->config->adminEmail, $this->config->siteTitle);
					$this->email->setTo($user->email);
					//$this->email->setTo('karthik.hbk24@gmail.com');
					$this->email->setSubject($this->config->siteTitle . ' - ' . 'Verify and Reset you Account');
					$this->email->setMessage($message);
					if ($this->email->send())
					{
						$this->setMessage('IonAuth.forgot_password_successful');
	 ...
	 * @author Michael
	 */
	public function signupPasswordCheck(string $code)
	{
		$user = $this->ionAuthModel->getUserBySignupPasswordCode($code);
		if (! is_object($user))
		{
			$this->setError('IonAuth.password_change_unsuccessful');
			return false;
